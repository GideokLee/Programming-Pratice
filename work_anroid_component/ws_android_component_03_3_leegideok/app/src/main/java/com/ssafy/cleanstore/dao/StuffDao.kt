...
class StuffDao {
    ...
    fun stuffSelect(stuffId: Int): Stuff? {
        sqlDB.rawQuery("""
            SELECT $STUFF_ID, $STUFF_NAME, $STUFF_CNT
            FROM $TABLE_NAME
            WHERE $STUFF_ID = ?
        """.trimIndent(), arrayOf(stuffId.toString())).use {
            it.moveToFirst()
            return Stuff(it.getInt(0),it.getString(1),it.getInt(2))
        }
    }
    ...
    fun stuffDelete(stuffId: Int): Int {
        val result = sqlDB.delete(TABLE_NAME, "$STUFF_ID = ?", arrayOf(stuffId.toString()))
        return result
    }
