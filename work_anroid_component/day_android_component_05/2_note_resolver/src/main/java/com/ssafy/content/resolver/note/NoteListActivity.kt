...
//        val result = mDbHelper.selectAllNotes()
        var result : MutableList<NotesDto> = mutableListOf()
        var queryUri = Notes.CONTENT_URI

        //정렬
        var sortOrder = Notes.DEFAULT_SORT_ORDER;
        ...
        contentResolver.query(queryUri, what, null, null, sortOrder)!!.use{
            if(it.moveToFirst()){
                do{
                    var id = it.getLong(it.getColumnIndex(Notes.ID))
                    var title = it.getString(it.getColumnIndex(Notes.TITLE))
                    var body = it.getString(it.getColumnIndex(Notes.BODY))
    ...
    private fun deleteNote(id:Long) {
        var uri = ContentUris.withAppendedId(Notes.CONTENT_URI, id)
